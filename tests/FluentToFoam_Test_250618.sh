...
MESH_FILE_REL="src/Openfoam/AeroSUV_mergedGeom_case/constant/triSurface/Cube_1mX1mX1m_FLuent_ascii.msh"
...
CONVERT_LOG="${LOG_DIR}/fluentMeshToFoam.log"
SOLVER_LOG="${LOG_DIR}/simpleFoam.log"
VTK_LOG="${LOG_DIR}/foamToVTK.log"
CHECK_LOG="${LOG_DIR}/sanity_check.log"

OPENFOAM_ENV="source /usr/lib/openfoam/openfoam2406/etc/bashrc"
...
for tool in fluentMeshToFoam simpleFoam foamToVTK; do
...
fluentMeshToFoam -scale 0.001 -writeZones -writeSets /workspace/${MESH_FILE_REL} \
  2>&1 | tee -a log/fluentMeshToFoam.log
