...
# === Check and copy wallDist to all processor folders ===
#WALLDIST_FILE="$CASE_DIR/constant/stream/wallDist"
#if [[ -f "$WALLDIST_FILE" ]]; then
#    echo "✅ wallDist file found at: $WALLDIST_FILE"
#    for ((i = 0; i < numProc; i++)); do
#        TARGET_DIR="$CASE_DIR/processor${i}/constant/stream"
#        mkdir -p "$TARGET_DIR"
#        cp "$WALLDIST_FILE" "$TARGET_DIR/wallDist"
#        echo "📁 Copied wallDist to: $TARGET_DIR/wallDist"
#    done
#else
#    echo "❌ wallDist file NOT found at: $WALLDIST_FILE"
#    echo "🛑 Simulation requires wallDist for kOmegaSST. Aborting."
#    exit 1
#fi
