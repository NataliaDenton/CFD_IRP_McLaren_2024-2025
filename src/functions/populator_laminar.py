...
def write_field_file(field_name: str, configU: dict, configA: dict, case_dir: str):
    ...
    if field_name == "omega" and configU["turbulence"]["model"].strip().lower() != "komegasst":
        print(f"ℹ️ Skipping 'omega' field — not required for model: {configU['turbulence']['model']}")
        return
...
def write_all_fields(configU: dict, configA: dict, case_dir: str):
    ...
    simulation_type = configU.get("turbulence", {}).get("simulationType", "").strip().lower()

    # List of turbulence fields that should not be included for laminar simulations
    turbulence_fields = {"k", "epsilon", "omega", "nut"}

    if simulation_type == "laminar":
        common_fields = fields_u.intersection(fields_a) - turbulence_fields
    else:
...
def populate_turbulenceProperties(ConfigU, ConfigA, case_dir):
    ...
    print(f"?? turbulence: simulationType = {simulation_type}")
    if simulation_type != "laminar":
        print(f"?? model = {ConfigU['turbulence'].get('model', 'NOT SET')}")


    case_dir = Path(case_dir).resolve()
    filepath = os.path.join('../../',case_dir, 'constant', 'turbulenceProperties')
    simulation_type = ConfigU['turbulence']['simulationType'].strip().lower()
    ...
    if simulation_type == "laminar":
        ...
        model_type = simulation_type.upper()  # "RAS" or "LES"
        model_name = ConfigU['turbulence'].get('model', 'kEpsilon')  # fallback in case missing


        # 💡 Fix: Use .get() to avoid KeyError if missing
        model_config = ConfigA.get(model_type, {})
        turbulence_value = "on" if model_config.get("turbulence", False) else "off"
        printCoeffs_value = "on" if model_config.get("printCoeffs", False) else "off"

        lines += [
            f"simulationType  {model_type};",
            f"{model_type}",
            "{",
            f"    {model_type}Model     {model_name};",
    ...
    print("✅ turbulenceProperties successfully populated.")'''
...
def generate_fvSchemes(fvSchemes_configU, fvSchemes_configA):
    divSchemes = fvSchemes_configA.get("divSchemes", {})

    div_entries = ""
    if "div(phi,U)" in divSchemes:
        div_entries += f"    div(phi,U)                      {divSchemes['div(phi,U)']};\n"
    if "div((nuEff*dev2(T(grad(U)))))" in divSchemes:
        div_entries += f"    div((nuEff*dev2(T(grad(U)))))  {divSchemes['div((nuEff*dev2(T(grad(U)))))']};\n"
...
def generate_fvSolution(fvSolution_configA):
    cacheAgglomeration = "on" if fvSolution_configA['solvers']['p']['cacheAgglomeration'] else "off"
        ...
        # Optional: k solver block
    k_solver_block = ""
    if "k" in fvSolution_configA["solvers"]:
        k_cfg = fvSolution_configA["solvers"]["k"]
        k_solver_block = f"""
    k
    {{
        solver          {k_cfg["solver"]};
        tolerance       {k_cfg["tolerance"]};
        relTol          {k_cfg["relTol"]};
        smoother        {k_cfg["smoother"]};
    }}"""


    # Optional: epsilon solver block
    epsilon_solver_block = ""
    if "epsilon" in fvSolution_configA["solvers"]:
        eps_cfg = fvSolution_configA["solvers"]["epsilon"]
        epsilon_solver_block = f"""
    epsilon
    {{
        solver          {eps_cfg["solver"]};
        tolerance       {eps_cfg["tolerance"]};
        relTol          {eps_cfg["relTol"]};
        smoother        {eps_cfg["smoother"]};
    }}"""

    # Optional: omega solver block
    omega_solver_block = ""
    if "omega" in fvSolution_configA["solvers"]:
        omega_cfg = fvSolution_configA["solvers"]["omega"]
        omega_solver_block = f"""
    omega
    {{
        solver          {omega_cfg["solver"]};
        tolerance       {omega_cfg["tolerance"]};
        relTol          {omega_cfg["relTol"]};
        smoother        {omega_cfg["smoother"]};
    }}"""
    
        # Optional: Phi solver block
    phi_solver_block = ""
    if "Phi" in fvSolution_configA["solvers"]:
        phi_cfg = fvSolution_configA["solvers"]["Phi"]
        phi_solver_block = f"""
    Phi
    {{
        solver          {phi_cfg["solver"]};
        tolerance       {phi_cfg["tolerance"]};
        relTol          {phi_cfg["relTol"]};
        smoother        {phi_cfg["smoother"]};
    }}"""

    
    

    # Residual control entries
    
    k_residual = ""
    if "k" in fvSolution_configA["SIMPLE"]["residualControl"]:
        k_residual = f'        k               {fvSolution_configA["SIMPLE"]["residualControl"]["k"]};'

    epsilon_residual = ""
    if "epsilon" in fvSolution_configA["SIMPLE"]["residualControl"]:
        epsilon_residual = f'        epsilon         {fvSolution_configA["SIMPLE"]["residualControl"]["epsilon"]};'

    omega_residual = ""
    if "omega" in fvSolution_configA["SIMPLE"]["residualControl"]:
        omega_residual = f'        omega           {fvSolution_configA["SIMPLE"]["residualControl"]["omega"]};'

    # Relaxation factors
    
    k_relax = ""
    if "k" in fvSolution_configA["relaxationFactors"]["equations"]:
        k_relax = f'        k               {fvSolution_configA["relaxationFactors"]["equations"]["k"]};'

    epsilon_relax = ""
    if "epsilon" in fvSolution_configA["relaxationFactors"]["equations"]:
        epsilon_relax = f'        epsilon         {fvSolution_configA["relaxationFactors"]["equations"]["epsilon"]};'

    omega_relax = ""
    if "omega" in fvSolution_configA["relaxationFactors"]["equations"]:
        omega_relax = f'        omega           {fvSolution_configA["relaxationFactors"]["equations"]["omega"]};'
