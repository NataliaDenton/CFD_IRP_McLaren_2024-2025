...
{{
    ...
    div(phi,epsilon)               {fvSchemes_configA["divSchemes"].get("div(phi,epsilon)", "Gauss upwind")};
    div(phi,omega)                 {fvSchemes_configA["divSchemes"].get("div(phi,omega)", "Gauss limitedLinear 1")};
    div((nuEff*dev2(T(grad(U)))))  {fvSchemes_configA["divSchemes"]["div((nuEff*dev2(T(grad(U)))))"]};
    div(div(phi,U))                {fvSchemes_configA["divSchemes"].get("div(div(phi,U))", "Gauss linear")};
}}
...
def generate_fvSolution(fvSolution_configA):
    ...
    # Optional: omega in residualControl
    omega_residual = ""
    ...
    # Optional: omega in relaxationFactors
    omega_relax = ""
